...
class Graph:
    def __init__(self, dataset, graph, labeling, part='body', max_hop=1, inter_link=False,
                 base=0, dilation=1, hop=1, **kwargs):
        ...
        self.hop = hop
        self.num_object = kwargs.get('num_object', 0)

        if labeling not in ['spatial', 'zeros', 'ones', 'eye', 'intra-inter', 'distance']:
            logging.info('')
            logging.error('Error: Do NOT exist this graph labeling: {}!'.format(labeling))
            raise ValueError()
    ...
    def _get_edge(self):
        ...
        if graph_base == 'coco':
            num_node = 17
            neighbor_link = self._coco_edges()
            center = 0
            connect_joint = self._coco_connect_joint()
            parts = [
                np.array([5, 7, 9]),
                np.array([6, 8, 10]),
                np.array([11, 13, 15]),
        ...
        elif graph_base == 'playground':
            num_obj = max(int(self.num_object), 0)
            num_node = 17 + num_obj
            neighbor_link = self._coco_edges()
            for obj_order in range(num_obj):
                obj_idx = 17 + obj_order
                neighbor_link.append((9, obj_idx))
                neighbor_link.append((10, obj_idx))
            center = 0
            connect_joint = np.concatenate(
                [self._coco_connect_joint(), np.array([10] * num_obj)]
            )
            if self.part == 'body':
                parts = [
                    np.array([5, 7, 9]),
                ...
                for obj_order in range(num_obj):
                    parts.append(np.array([17 + obj_order]))
            elif self.part == 'person':
                body_nodes = np.arange(17)
                object_nodes = np.arange(17, 17 + num_obj) if num_obj else np.array([], dtype=int)
                parts = [body_nodes]
                if object_nodes.size:
                    parts.append(object_nodes)
            else:
                parts = [np.arange(num_node)]
        elif graph_base == 'coco_ball':
            num_node = 18
            neighbor_link = self._coco_edges() + [(9, 17), (10, 17)]
            center = 0
            connect_joint = np.concatenate([self._coco_connect_joint(), np.array([10])])
            if self.part == 'body':
                parts = [
                    np.array([5, 7, 9]),
                    np.array([6, 8, 10]),
                ...
                parts = [
                    np.arange(num_node - 1),
                    np.array([num_node - 1]),
                ]
        elif graph_base == 'coco_ball_net':
            num_node = 19
            neighbor_link = self._coco_edges() + [(9, 17), (10, 17), (17, 18)]
            center = 0
            connect_joint = np.concatenate([self._coco_connect_joint(), np.array([10, 17])])
            if self.part == 'body':
                parts = [
                    np.array([5, 7, 9]),
                    np.array([6, 8, 10]),
            ...
            parts = [
                np.array([5, 6, 7]),
                np.array([2, 3, 4]),
                np.array([9, 10, 11, 22, 23, 24]),
                np.array([12, 13, 14, 19, 20, 21]),
                np.array([0, 1, 8, 15, 16, 17, 18]),
            ]
            center = 1
        else:
            logging.error('Unsupported graph: {}'.format(graph_base))
            raise ValueError()
        ...
        if num_person > 1:
            ...
            for i in range(num_person):
                ...
                for x, y in neighbor_link:
                    neighbor_link_intra_nperson.append((x + i * num_node, y + i * num_node))
                if self.part == 'body':
            ...
            if self.inter_link == 'linear':
                ...
                for base_node in bases:
                    for i in range(h, m, d):
                        neighbor_link_inter_nperson.append((i * num_node + base_node, (i - self.hop) * num_node + base_node))

            num_node *= num_person
            neighbor_link = neighbor_link_intra_nperson + neighbor_link_inter_nperson
            connect_joint = connect_joint_nperson
            parts = parts_nperson if parts_nperson else [np.arange(num_node)]
        ...
        if self.labeling == 'distance':
            ...
            for hop in valid_hop:
                ...
                for i in range(self.num_node):
                    for j in range(self.num_node):
                        if hop_dis[j, i] == hop:
                            ...
                            elif hop_dis[j, self.center[i]] == hop_dis[i, self.center[i]]:
                                a_root[j, i] = normalize_adjacency[j, i]
                            elif hop_dis[j, self.center[i]] < hop_dis[i, self.center[i]]:
                ...
                if hop == 0:
                    ...
                    A.append(a_further)
            A = np.stack(A) if len(A) else np.zeros((0, self.num_node, self.num_node))
